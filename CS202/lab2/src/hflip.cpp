...
int main()
{
	int rows, cols, tmpi, i, j; //define rows, columns, temporary variables
	int counter = 0; //initialize a counter at 0
	string tmps; //temporary string
	vector <int> hflip, temp; //vector to hold flipped pixels
	...
	for(i = 0; i < rows; i++) //for loop to check all pixels
	{
		for(j = 0; j < cols; j++)
		{
			...
			{
				temp.push_back(tmpi);
				counter++; //keeps track of location in file
			}
		}
		for(j = (int) temp.size() - 1; j >= 0; j--)
		{
			hflip.push_back(temp[j]);
		}
		temp.clear();
	}
    ...
    for(i = 0; i < (int) hflip.size(); i++) //prints flipped pixels
    {
        cout << hflip[i] << endl;
    }
